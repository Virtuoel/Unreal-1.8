...
public class EntityTranslocatorDisc extends EntityProjectileUnreal
{
	...
	public void onUpdate()
	{
		...
		if(this.isBurning())
		{
	        this.worldObj.playSoundEffect(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "random.fizz", 0.5F, 2.6F + (this.worldObj.rand.nextFloat() - this.worldObj.rand.nextFloat()) * 0.8F);
			this.setDead();
		}
		...
		{
            double d0 = this.posX +        (double)(rand.nextFloat() - 0.5F) * 0.2D;
            double d1 = this.posY + 0.3F + (double)(rand.nextFloat() - 0.5F) * 0.2D;
            double d2 = this.posZ +        (double)(rand.nextFloat() - 0.5F) * 0.2D;
            this.worldObj.spawnParticle(EnumParticleTypes.REDSTONE, d0, d1, d2, 0.0D, 0.0D, 0.0D, new int[0]);
		...
		else
		{
			...
			if (movingobjectposition != null)
			{
				if (movingobjectposition.entityHit != null)
				{
					...
					{
						damagesource = (new EntityDamageSourceIndirect(Reference.MOD_ID + "." + Reference.DamageSource.TRANSLOCATOR, this, this)).setProjectile();
					}
					else
					{
						damagesource = (new EntityDamageSourceIndirect(Reference.MOD_ID + "." + Reference.DamageSource.TRANSLOCATOR, this, this.shootingEntity)).setProjectile();
					}
